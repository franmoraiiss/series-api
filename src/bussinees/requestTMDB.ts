...
async function createEpisode(
  ...
  try {
    ...
    const episode = await axios.get(
      `https://api.themoviedb.org/3/tv/${idSerie}/season/${season_number}/episode/${episodeNumber}?api_key=${api_key}`,
    );
...
async function createSeason(
  ...
  try {
    ...
    const seasons = await axios.get(
      `https://api.themoviedb.org/3/tv/${idSerie}/season/${season_number}?api_key=${api_key}`,
    );
    ...
    if (verSeason != null) {
      ...
      console.log(
        "------ Cadastrando os Episodios da temporada ",
        season_number,
        " e Associando Elenco da serie ------------",
      );
      for (const episode in seasons.data.episodes) {
        //Cria o episodio

        await createEpisode(
          seasons.data.episodes[episode].episode_number,
          idSerie,
          season_number,
          seasons.data.id,
        );
...
async function createRelationPlataformaSerie(
  idPlataforma: number,
  idSeason: number,
) {
  try {
...
async function createRelationGenreSerie(
  idGenre: number,
  idSerie: number,
  nameGenre: string,
) {
  ...
  //Se não existir, cadastra
    ...
    try {
      ...
      const response = await axios.get(
        `https://api.themoviedb.org/3/tv/${idSerie}?api_key=${api_key}`,
      );
      ...
      console.log(
        "------ Cadastrando os criadores da serie: ",
        response.data.name,
        " ------------",
      );
      ...
      for (const season in response.data.seasons) {
        await createSeason(
          response.data.seasons[season].season_number,
          response.data.id,
          response.data.seasons[season].episode_count,
        );
      }
      ...
      for (const genre in response.data.genres) {
        ...
        createRelationGenreSerie(
          idGenre,
          idSerie,
          response.data.genres[genre].name,
        );
      }
      ...
      for (const network in response.data.networks) {
        await createPlataforma(
          response.data.networks[network].id,
          response.data.networks[network].name,
        );
        createRelationPlataformaSerie(
          response.data.networks[network].id,
          idSerie,
        );
      }
...
export class GetSeriesTmdb {
  async handle(request: Request, response: Response) {
    ...
    console.log(
      "------ Carregando os Generos existentes para o banco ------------",
    );
    ...
    try {
      ...
      for (let pageAt = page; pageAt <= maxPage; pageAt++) {
        ...
        // await new Promise(r => setTimeout(r, 500));
        const response = await axios.get(
          `https://api.themoviedb.org/3/tv/popular?api_key=${api_key}&page=${pageAt}`,
        );
        ...
        for (const series in arraySeries) {
          ...
          if (confirm == 1) {
            console.log(
              "------ Cadastrando os trailers da serie ",
              arraySeries[series].name,
              " ------------",
            );
